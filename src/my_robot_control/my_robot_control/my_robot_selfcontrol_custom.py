...
class RobotSelfControl(Node):
    ...
    def laser_callback(self, scan):
        ...
        # Determine zone
        if -60 <= angle_closest_distance <= 60:
            zone = "FRONT"
        elif 60 < angle_closest_distance <= 110:
            zone = "LEFT"
        elif -110 <= angle_closest_distance < -60:
            zone = "RIGHT"
        ...
        if closest_distance < self._distanceLimit:
            if zone == "FRONT":
                ...
                self._msg.angular.z = self._forwardSpeed * self._speedFactor * 2
            elif zone == "LEFT":
                self._msg.linear.x = 0.0
